...
def create_augmented_dataloader(args, dataset):
    ...
    original_tokenized = tokenized_dataset["train"]

    random_indices = random.sample(range(len(dataset["train"])), 5000)
    sampled = dataset["train"].select(random_indices)
    ...
    combined = datasets.concatenate_datasets([original_tokenized, transformed_tokenized])
    train_dataloader = DataLoader(combined, shuffle=True, batch_size=args.batch_size)
